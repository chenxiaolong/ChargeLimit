...
val projectUrl = "https://github.com/chenxiaolong/ChargeLimit"
...
android {
    namespace = "com.chiller3.chargelimit"
    ...
    defaultConfig {
        applicationId = "com.chiller3.chargelimit"
        minSdk = 35
        targetSdk = 35
...
var chargeLimitSelinuxTasks = mutableMapOf<String, TaskProvider<Exec>>()
...
for ((target, abi) in listOf(
    ...
    val chargeLimitSelinux = tasks.register<Exec>("chargeLimitSelinux$suffix") {
        val srcDir = File(rootDir, "chargelimit-selinux")
        ...
        outputs.files(
            File(File(File(File(rootDir, "target"), target), "release"), "chargelimit-selinux")
        )
    ...
    chargeLimitSelinuxTasks[abi] = chargeLimitSelinux
}
...
android.applicationVariants.all {
    ...
    val moduleProp = tasks.register("moduleProp${capitalized}") {
        ...
        doLast {
            ...
            props["description"] = "Battery charge limit quick settings tile for Google Pixel devices"
    ...
    tasks.register<Zip>("zip${capitalized}") {
        ...
        for ((abi, task) in chargeLimitSelinuxTasks) {
            from(task.map { it.outputs }) {
                rename { "chargelimit-selinux.${abi}" }
            }
        }
